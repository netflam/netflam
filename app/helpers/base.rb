class Netflam
  ...
  def session
    env["rack.session"] || raise(RuntimeError,
      ...
      "by adding Netflam.use Rack::Session::Cookie")
  end
  ...
  # @example
  #   def redirect(*args)
  #     run Netflam.new { on(default) { res.redirect(*args) }}
  #   end
  #
  #   on "account" do
...
Netflam.settings[:req] = Rack::Request
Netflam.settings[:res] = Netflam::Response